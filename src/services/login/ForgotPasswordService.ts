...
const sendOtp = async (forgotemail :string ): Promise<AuthResponse> => {
  ...
  try {
    const response = await axios.post(`https://g23jza8mtp.ap-south-1.awsapprunner.com/applicant/forgotpasswordsendotp`, {
...
const resetPassword = async (email:string,password:string,confirmedPassword:string):Promise<AuthResponse> => {
  try {
    ...
    const response = await axios.post(
        `https://g23jza8mtp.ap-south-1.awsapprunner.com/applicant/applicantreset-password/${email}`,
        {  password,
           confirmedPassword
